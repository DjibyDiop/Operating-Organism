...
function Resolve-ImagePath([string]$override) {
  ...
  # Search both scripts/ dir (legacy) and repo root (current build output).
  $repoRoot = Split-Path -Parent $PSScriptRoot
  $candidates = @()
  foreach ($searchDir in @($PSScriptRoot, $repoRoot)) {
    $found = Get-ChildItem -Path $searchDir -Filter 'llm-baremetal-boot*.img' -ErrorAction SilentlyContinue |
...
function Build-QemuArgs([string]$accelMode, [bool]$forceAvx2) {
  ...
  # For TCG, use 'max' too to expose AVX2/FMA (avoids #UD on EFI AVX2 code).
  $cpuModel = 'qemu64'
  if ($accelMode -eq 'whpx' -or $accelMode -eq 'tcg') { $cpuModel = 'max' }
