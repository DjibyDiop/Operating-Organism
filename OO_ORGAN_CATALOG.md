...
| Brain Cortex | Planning, reasoning | Long-horizon planning and interpretation via SSM | `PlannerEngine`, `ReasoningEngine` | `OPI-baremetal` (thalamic-bloom) | `oo-host`, `oo-model` |
| Brainstem | Vital reflexes | Hard safety path and survival fallback | `ReflexEngine`, `SafeModeEngine` | `reflex-baremetal`, `kernel-baremetal` | `oo-constitution` |
...
| Blood (RBC/WBC/Plasma) | Transport + immune patrol | Event transport (RED=data, WHITE=immune, YELLOW=energy) | `EventTransport`, `ImmunePatrol`, `ContextCarrier` | `united-baremetal` (globule types) | `oo-constitution`, `oo-host` |
...
| Hepatic System (Liver) | Filter toxins, regulate chemistry | Policy filtering via 5 Organic Laws (D+ Policy Engine) | `PolicyFilterEngine`, `SanitizationEngine` | `OPI-baremetal/thalamic-bloom` (D+ bridge) | `oo-constitution` |
...
| Immune System | Detect and isolate threats | Integrity checks, anomaly detection, quarantine | `IntegrityGuardEngine`, `AnomalyDetectorEngine`, `QuarantineEngine` | `bot-baremetal`, `shadow-baremetal` | `oo-constitution`, `oo-host` |
| Endocrine System | Hormonal regulation | Global mode signaling and adaptive thresholds | `ModeSignalEngine`, `AdaptiveThresholdEngine` | `vital-baremetal` (hormone levels) | `oo-system`, `oo-constitution` |
...
| Integumentary System (Skin) | Boundary and protection | External boundary, trust zone ingress control | `BoundaryControlEngine`, `IngressGuardEngine` | `shadow-baremetal` (anti-forensics) | `oo-constitution` |
...
| Reproductive/Mutation System | Variation over time | Controlled evolution and module mutation workflow | `MutationGovernanceEngine`, `VariantEvalEngine` | `evolution-baremetal`, `regen-baremetal` | `oo-constitution`, `oo-model` |
...
| Identity / DNA | Self-recognition | Hardware fingerprint, TPM, FNV-1a DNA hash | `IdentityEngine`, `DNAHashEngine` | `identity-baremetal` | `oo-constitution` |
...
- **Adaptive Organs**: cortex (`OPI-baremetal` substrate, `OPI` cellular mind), endocrine (`vital-baremetal`), digestive (`sense-baremetal`), mutation (`evolution-baremetal`, `regen-baremetal`).
...
| `OPI-baremetal` | Brain Cortex Substrate (Llama 3.2 Q8_0 baremetal engine) | Adaptive |
| `OPI` | Cellular Mind Core (OMC, OPC, OCC, OLE, OPE, CVM & Compiler District) | Adaptive / Vital |
